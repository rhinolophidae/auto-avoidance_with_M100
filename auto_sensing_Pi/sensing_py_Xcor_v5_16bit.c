	...
	char filename[256];
		...
		for(k = 0;k < DataNum;k++)
			{	
				...
				if (receiver_side == "R"){//*ここが左耳の条件　ここをかえよう
					fprintf(fp, "%f\n", rxwfm[i]);
				}
...
void extract_peaks(char* receiver_side, int rightpeaks[BOXNUM], int leftpeaks[BOXNUM])							//左右のピークを区別するために引数を導入
	{
	int i,maxflag;
	double 	max=0.,thd, thd_base, xp, yp, t_rest;
	FILE *fp;
	
	thd_base=1900000000; //10ms de 300000 no keisan
	t_rest=250.0;
...
//	printf("thresh value %lf \n\n", thd);
	for (i=1; i<BOXNUM/2-1; i++)		{							//# BOXNUM to BOXNUM/2
		...
		if ( ( (crwv[i]-crwv[i-1]) > 0.) && ( (crwv[i+1]-crwv[i]) < 0.)  && ( crwv[i] > thd )  && (i > direct_pulse_time ))	//#
			...
			if (receiver_side == "L"){
				if (lflag >= 1 && i-leftpeaks[lflag-1] <= t_rest){
					if(crwv[i] / max > leftpeaks_val[lflag-1]){
						leftpeaks[lflag-1] = i;
						leftpeaks_val[lflag-1] = crwv[i]/max;
						printf("updt_left %d us\n", leftpeaks[lflag-1]);
					}
				}else{
					leftpeaks[lflag] = i;	
					leftpeaks_val[lflag] = crwv[i]/max;	
					printf("left %d us\n", leftpeaks[lflag]);				
					lflag++;
				}
			}else{
				if (rflag >= 1 && i-rightpeaks[rflag-1] <= t_rest){
					if(crwv[i] / max > rightpeaks_val[rflag-1]){
						rightpeaks[rflag-1] = i;
						rightpeaks_val[rflag-1] = crwv[i]/max;
						printf("updt_right %d us\n", rightpeaks[rflag-1]);
					}
				}else{
					rightpeaks[rflag] = i;
					rightpeaks_val[rflag] = crwv[i]/max;	
					printf("right %d us\n", rightpeaks[rflag]);			
					rflag++;
				}
			}
...
*/		fprintf(stdout,"peak_det_R: %d_%d_%d\n", rightpeaks[0],rightpeaks[1],rightpeaks[2]);
