...
#define BOXNUM  16384   //  data points for calculating array
#define DataNum 16384
...
void tx_wfm_load()     // load tx waveform file(pulse drive signal) pulse_drive.dat 
	...
	if (NULL == ( fp=fopen("pulse_drive.dat","r")))
	...
	if (NULL == ( fp=fopen("result_sensing/txwv/chk_txwv.dat","w"))){
...
void pulse_aux_load()     //
	...
	if (NULL == ( fp=fopen("pulse_drive_aux.dat","r")))
...
void make_rx(char* receiver_side, int mes_index)//生波形データを変数に取り込む(TRIGTSDに作用されなくした)。
{
	...
	char filename[256];
		...
		if (receiver_side == "L"){
			sprintf(filename, "result_sensing/rxwv/chk_rxwv_left_%d.dat", mes_index);
			fp =fopen(filename, "w");
		}
		else{
			sprintf(filename, "result_sensing/rxwv/chk_rxwv_right_%d.dat", mes_index);			
			fp =fopen(filename, "w");
		}
...
int main(int argc, char *argv[]){
    ...
    ////////////////////ここまでがFPGA initial処理
		...
		int mes_index = 0;
		for(mes_index=0; mes_index<mes_num; mes_index++) {
      		make_rx("R", mes_index);
			make_rx("L", mes_index);
		}
}
