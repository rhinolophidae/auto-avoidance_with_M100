...
void make_rx(char* receiver_side, int mes_index)//生波形データを変数に取り込む(TRIGTSDに作用されなくした)。
{
	...
	char filename[256];
		...
		if (receiver_side == "L"){
			sprintf(filename, "result_sensing/%d_%d_%d/rxwv/chk_rxwv_left_%d.dat", f0, f1, T, mes_index);
			fp =fopen(filename, "w");
		}
		else{
			sprintf(filename, "result_sensing/%d_%d_%d/rxwv/chk_rxwv_right_%d.dat", f0, f1, T, mes_index);
			fp =fopen(filename, "w");
		}
...
int main(int argc, char *argv[]){
    ...
    ////////////////////ここまでがFPGA initial処理
    for(int loop_n=0; loop_n<20; loop_n++) {
...
//		  make_rx("R", y);
//		  make_rx("L", y);
		  system("python3 crwv_plot_1time_real_v1.py ok go");
		}
