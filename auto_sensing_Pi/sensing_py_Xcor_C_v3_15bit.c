...
void make_rx(char* receiver_side, int mes_index, double rxwfm[BOXNUM], int loop_n)//生波形データを変数に取り込む(TRIGTSDに作用されなくした)。
{
	...
	char filename[256];
		...
		if (receiver_side == "L"){
			sprintf(filename, "result_sensing/%d_%d_%d/rxwv/chk_rxwv_left_%d.dat", f0, f1, T, loop_n);
			fp =fopen(filename, "w");
		}
		else{
			sprintf(filename, "result_sensing/%d_%d_%d/rxwv/chk_rxwv_right_%d.dat", f0, f1, T, loop_n);
			fp =fopen(filename, "w");
		}
